...
$e.fn.picSelector = function (opt) {
    ...
    function add(pic) {
        ...
        var p = $e(config.template.replace("%URL%", pic.url)).appendTo(config.container);
        if (pic.clip) {
...
//--------------------------------------------------------------------------------------------------
//图文编辑器，两种用法：
//1.var editor = $e(selector).editor(opt); selector为隐藏域、文本框等，初始内容为隐藏域、文本框的内容，也可为html(字符串）替换
//2.var editor = $e.editor(opt);
//      opt = {
...
//--------------------------------------------------------------------------------------------------
$e.editor = $e.fn.editor = function (opt) {
    var _this = $e(this);
    var config = {
        container: null,
    };
    ...
    var box = $e('<div class="content editor"></div>').appendTo(config.container);
    var toolbar = $e('<div class="menu box box-horizontal">' +
                    ...
                    '</div>').appendTo(box);
    var editor = $e('<div class="text-area" contenteditable="true"' + (config.height > 0 ? ' style="height:' + config.height + 'px"' : '') + '></div>').appendTo(box);
...
function clipImage(url, w, h, callback) {
    var ww = $e(window).width(), wh = $e(window).height() - 50;
    var iw = 0, ih = 0;
    ...
    var back = $e('<div style="position:fixed;top:0;left:0;width:100%;height:100%;background:#fff;z-index:1000000"></div>').appendTo($e('body'));
    var img = $e('<img src="' + url + '" style="position:fixed;z-index:1000001" />').appendTo($e('body')).bind('load', function () {
        iw = $e(this).width() * zoom;
        ih = $e(this).height() * zoom;
        $e(this).css({ left: (ww - iw) / 2, top: (wh - ih) / 2, width: iw, height: ih });
    });
    var view = { x: parseInt((ww - w * zoom) / 2) * zoom, y: parseInt((wh - h * zoom) / 2) * zoom, width: w * zoom, height: h * zoom };
    var cover = $e('<div style="position:fixed;top:0;left:0;width:100%;height:100%;box-sizing: border-box;border:1px solid rgba(0,0,0,0.8);z-index:1000002; border-width:' + view.y + 'px ' + view.x + 'px ' + (view.y + 50) + 'px;"></div>')
    .appendTo($e('body')).bind('touchstart', function (e) {
        e.preventDefault();
        var ts = e.touches;
    ...
    var buttons = $e('<div style="position:fixed;width:100%;bottom:0;left:0;z-index:1000003">' +
        ...
        '</div>').appendTo($e('body')).find('a').click(function () {
            if ($e(this).html() == '确定') {
...
function getLocation(longitude, latitude, callback) {
    function createMap(lng, lat) {
        ...
        var container = $e('<div style="width:100%;height:100%;position:absolute;z-index:1000001" id="baidu_map"></div>').appendTo($e('body'));
        var map = new BMap.Map('baidu_map');
        var point = new BMap.Point(p.lng, p.lat);
        map.centerAndZoom(point, 18);
        map.addControl(new BMap.NavigationControl());

        var cross = $e('<div style="position:absolute;width:40px;height:40px;z-index:1000002;left:50%;top:50%;margin-left:-20px;margin-top:-40px;background:url(/static/images/locate.png) no-repeat center;background-size:100%"></div>').appendTo($e('body'));
...
function getAMapLocation(longitude, latitude, callback) {
    function createMap(lng, lat) {
        var container = $e('<div style="width:100%;height:100%;position:absolute;z-index:1000001" id="gaode_map"></div>').appendTo($e('body'));
        var map, geolocation;
        /*加载地图，调用浏览器定位服务*/
...
var ecwx = {
    ...
    appType: 0,
    ext: {},
    imgTemplate: '<li><i class="trashIcon" onclick="$e(this.parentNode).remove();"></i><a href="javascript:void(0);"><img class="pay_pic" src="${_.url}" alt=""/></a></li>',
    imgsTemplate: '{@each _.imgs as img,indx}<li><i class="trashIcon" onclick="$e(this.parentNode).remove();"></i><a href="javascript:void(0);"><img class="pay_pic" src="${img}" alt=""/></a></li>{@/each}',
    init: function (param) {
    ...
    initImage: function (ipara) {
        ...
        if (window.wx) {
            ...
            wechat.init(function () {
                ecwx.ps = $e(selector).picSelector(config);
            });
        }
    },
    ...
    onAppImageCompleted: function (imgUrls) {
        if (imgUrls == undefined || imgUrls == '') {
            ...
            var json = $e.parseJSON(imgUrls);// { imgs: imgUrls } 
